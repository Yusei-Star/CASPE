...
class FastaBatchedDataset(object):
    def __init__(self, sequence_names, sequence_labels, sequence_strs):
    ...
    def from_file(cls, fasta_file):
        sequence_names, sequence_labels, sequence_strs = [], [], []
        cur_seq_label = None
        cur_seq_name = None
        buf = []

        def _flush_current_seq():
            nonlocal cur_seq_name, cur_seq_label, buf
            if cur_seq_label is None or cur_seq_name is None:
                return
            sequence_labels.append(cur_seq_label)
            sequence_names.append(cur_seq_name)
        ...
        with open(fasta_file, "r") as infile:
            for line_idx, line in enumerate(infile):
                if line.startswith(">"):  # label line
                    ...
                    line = line[1:].strip()
                    if len(line) > 0:
                        cur_seq_name = line.split("_")[0]
                        cur_seq_label = int(line.split("_")[-1])
                else:  # sequence line
        ...
        return cls(sequence_names, sequence_labels, sequence_strs)

    def __len__(self):
        return len(self.sequence_labels)

    def __getitem__(self, idx):
        return self.sequence_names[idx], self.sequence_labels[idx], self.sequence_strs[idx]
...
class Alphabet(object):
    ...
    def encode(self, text):
        return [self.tok_to_idx[tok] for tok in self.tokenize(text)] # get seq toks
...
class BatchConverter(object):
    ...
    def __call__(self, raw_batch: Sequence[Tuple[str, str]]):
        ...
        batch_names, batch_labels, seq_str_list = zip(*raw_batch)
        ...
        for i, (name, label, seq_str, seq_encoded) in enumerate(
            zip(batch_names, batch_labels, seq_str_list, seq_encoded_list)
        ):
            names.append(name)
            labels.append(label)
            strs.append(seq_str)
            ...
            tokens[
                i,
                int(self.alphabet.prepend_bos) : len(seq_encoded) + int(self.alphabet.prepend_bos), # plus 0 and 2
            ] = seq
        ...
        return names, labels, strs, tokens # generate [0, seq, 2]
...
class MSABatchConverter(BatchConverter):
    def __call__(self, inputs: Union[Sequence[RawMSA], RawMSA]):
        ...
        for i, msa in enumerate(raw_batch):
            ...
            msa_names, msa_labels, msa_strs, msa_tokens = super().__call__(msa)
            names.append(msa_names)
            labels.append(msa_labels)
            strs.append(msa_strs)
            tokens[i, : msa_tokens.size(0), : msa_tokens.size(1)] = msa_tokens

        return names, labels, strs, tokens
